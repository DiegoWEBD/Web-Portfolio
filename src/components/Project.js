const Project = ({ project }) => {
    ...
    return(
        <div className='w-[440px] flex justify-center'>
            <div className="opacity-70 w-[350px] hover:w-[368px] sm:w-[400px] sm:hover:w-[418px] hover:opacity-100 block pt-6 pb-4 mb-12 px-[26px] hover:px-[35px] h-fit transition-all  rounded-lg border shadow shadow-black bg-slate-50 hover:shadow-xl hover:h-full hover:rounded-xl">
                ...
                <div className="mt-4 break-words">
                    <p className="mb-1 text-2xl text-black font-extrabold transition-all duration-300">{ project.name }</p>
                    <p className="mb-4 text-[15px] text-slate-800 font-semibold italic transition-all duration-300">{ project.description }</p>

                    {
                        project.github_link
                            ?   <div className="mb-1 flex">
                                    <p className="text-md my-auto text-indigo-700 font-bold mr-3 transition-all duration-300 ">Github:</p>
                                    <p className="text-md my-auto hover:text-slate-500 font-semibold transition-all duration-300"><a rel='noopener noreferrer' href={project.github_link} target="_blank"><img alt={project.github_link} className='h-5' src={ require('../img/github_logo.png') } /></a></p>
                                </div>
                            :   null
                    }
                    {
                        project.page_link
                            ?   <div className="mb-1 flex">
                                    <p className="text-xs text-blue-700 font-bold mr-3 transition-all duration-300">Web:</p>
                                    <p className="text-xs text-blue-500 hover:text-blue-700 font-semibold transition-all duration-300"><a rel='noopener noreferrer' href={project.page_link} target="_blank">{project.page_link}</a></p>
                                </div>
                            :   null
                    }
